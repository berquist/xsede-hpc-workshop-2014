...
#define ROWS ROWS_GLOBAL/npes
    ...
    // verify only even numbers  PEs are being used
    if (npes % 2 != 0) {
      if (my_PE_num == 0)
	printf("This code must be run with and even number of PEs.\n");
      MPI_Finalize();
      exit(1);
    }
